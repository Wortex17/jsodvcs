...
//Tracked for future updates to know which repository have comparative hashes
let hashVersion = "object-hash@1.1.2";
...
class Repository
{
    ...
    constructor(options){
        ...
        /**
         * The client configuration for the jsod content diff/merge tool.
         * @type {{diff: Object, merge: Object}}
         */
        this.jsodConfig = {
            diff: {},
            merge: {}
     ...
     * WIP (very simple) version of fetch().
     * Copies all remote branches and objects into this repository, in preparation for merging.
     * Used by pull and clone.
     *
     * If a remote branch is beeing tracked during fetching (atomatically or via the option), FETCH_HEAD will be set,
     * users can call merge(FETCH_HEAD) to directly merge the fetched history.
     * @param {Repository} remote
     * @param {Object} [options]
     * @param {string} [options.name = "origin"] - The name to treat the given remote with
     * @param {string} [options.trackedBranch] - If given, overrides which remote branch is set as matching branch for the local one.
     * @param {Array.<string>} [options.branches] - If given, determines which branches are to be fetched. Defaults to all.
     *
     * @todo Only sync required objects (required by the branches to be fetched).
     */
    fetch(remote, options)
    {
        if(!(remote instanceof Repository))
        {
            throw new TypeError("Cannot fetch from non-Repository remote: " + remote);
        }
        if(remote.isMerging)
        {
            throw new Error("Cannot fetch from remote which is merging: " + remote);
        }
        if(this.isMerging)
        {
            throw new Error("Cannot fetch from remote while merging");
        }

        options = _.extend({
            name: "origin",
            trackedBranch: this.currentBranch
        }, options);

        let that = this;
        this.fetch_objects(remote);

        let theirBranches = remote.list_branches();

        //If branches are given, filter theirBranches
        if(_.isArray(options.branches))
            theirBranches = _.pick(theirBranches, options.branches);

        let remoteName = options.name;
        _.forEach(theirBranches, function(pointedCommit, branch){
            let branchRef = "refs/remotes/" + remoteName + "/" + branch;
            that.set_ref(branchRef, pointedCommit);
            if(branch == options.trackedBranch)
            {
                that.FETCH_HEAD = branchRef;
            }
        });
    ...
    fetch_objects(remote)
    {
        if(remote.hashVersion != this.hashVersion)
        {
            throw new Error("Fetching from remote with a different hash version is currently unsupported: " + remote);
        }

        let that = this;
        //Sync ALL objects for now
        let theirObjects = remote.list_objects();
        _.forEach(theirObjects, function(stored, objectHash){
            that.objects.set(objectHash, _.cloneDeep(stored.content), {
    ...
    _diff3(treeX, treeA, treeB)
    {
        ...
        _.forEach(diffB, function(path){
            ...
            unidiff[path] =
                //If there was no modificationA or it is the same as B, there is no index conflict.
                //Do not check if modificationA is undefined! Undefined is a valid entry in the unidiff and
                //marks deleted content.
                (!unidiff.hasOwnProperty(path) || modificationA == modificationB)
                    ? modificationB
     ...
     * @param {Date} [options.date]  - The author of this commit. Defaults to Date.now().
     * @param {boolean} [options.ignoreConflicts = false]  - Ignore if the repository has merge conflicts and commit away
     ...
     * @param {boolean} [options.out.didCommit]  - Outgoing field where it is stored if a new commit was created
     *
     * @throws {Error} When trying to commit while there are merge conflicts.
     *
     * @return {jsodvcs.Repository}
     */
     ...
     * @param commit_ish
     * @param {Object} [options]
     * @param {boolean} [options.indexOnly = false] - If true, do not generate content conflicts (only index conflicts)
     * @param {boolean} [options.tryAutoResolve = true] - If false, do not try to auto resolve index conflicts
     * @return {Repository}
     */
            ...
            if(!options.indexOnly)
            {
                _.forIn(that.mergeConflicts, function(mergeConflict){
                    ...
                    mergeConflict.contentDelta = jsod.diff3(
                        ...
                        that.jsodConfig.diff, //diffConfig
                        that.jsodConfig.merge, //mergeConfig
                        mergeConflict.contentDeltaConflictNodes
                    );
                });
            }
     ...
     * Basically a shorthand for fetch() followed by merge(FETCH_HEAD)
     * Targets the remote branch that matches/is tracked the local branch, if not ovveridden by the branch
     * option.
     * @param {Repository} remote
     * @param {Object} [options]
     * @param {string} [options.name = "origin"] - The name to treat the given remote with
     * @param {string} [options.branch] - If given, overrides which remote branch shoudl be pulled into the local branch
     */
    pull(remote, options)
    {
        options = _.extend({
        ...
        this.fetch(remote, {
            ...
            trackedBranch: options.branch
        });
        this.merge(this.FETCH_HEAD);
        return this;
    }

    /**
     * Update a remote repository with local history.
     * Pushes all changes of local branches, that are ahead of changes on matching
     * remote branches to the remote repository.
     * Beware that this command will interact with the remote repository, so it my change its objects,
     * refs and index.
     ...
     * @param {Array.<string>} [options.out.didRemoteCheckout] - Was the remote checkedOut in the process? This happens when the current branch of the remote is being pushed to.
     * @param {Array.<string>} [options.out.pushedBranches] - Outgoing list of branches that have been pushed successfully.
     * @param {Array.<string>} [options.out.upToDateBranches] - Outgoing list of branches that have been ignored because the remote already was up to date.
     * @param {Array.<string>} [options.out.rejectedBranches] - Outgoing list of branches that have been rejected (e.g. because the local branch was behind the remote one)
     */
    push(remote, options)
    {
        options = _.extend({
        ...
        options.out.pushedBranches = [];
        options.out.upToDateBranches = [];
        options.out.rejectedBranches = [];

        if(!(remote instanceof Repository))
        {
            throw new TypeError("Cannot push to non-Repository remote: " + remote);
        }

        let remoteHasFetchedAllObjects = false;
        let remoteCurrentBranch =  Repository.treatAsBranchName(remote.currentBranch);

        options.out.didRemoteCheckout = false;
        let branchesToPush = this.list_branches();
        _.forIn(branchesToPush, function(branchHead, branch){
            branch = Repository.treatAsBranchName(branch);

            let mineBranchCommit = this.resolve_ref(branch);
            let theirBranchCommit = remote.resolve_ref(branch);

            //Check if their branch is ahead of ours.
            let lca = this.get_lca(mineBranchCommit, theirBranchCommit);
            let isAhead = _.isUndefined(theirBranchCommit) || lca == theirBranchCommit;

            if(mineBranchCommit == theirBranchCommit)
            {
                options.out.upToDateBranches.push(branch);
            }
            else if(isAhead)
            {
                if(!remoteHasFetchedAllObjects)
                {
                    remote.fetch_objects(this);
                    remoteHasFetchedAllObjects = true;
                }
                remote.set_ref(branch, branchHead);
                options.out.pushedBranches.push(branch);
                if(branch == remoteCurrentBranch)
                {
                    //So the remote reflects its correct state, it has to checkout to its (updated) branch.
                    remote.checkout(branch);
                    options.out.didRemoteCheckout = true;
                }
            } else {
                options.out.rejectedBranches.push(branch);
            }
        }.bind(this));
        return this;
    }
    ...
    clone()
    {
        ...
        args.unshift(this);
        return Repository.clone.apply(this, args);
    }
}
/**
 * Clone a repository
 ...
 * @return {Repository}
 */
Repository.clone = function(remote){
    //With the current implementation, a cloneDeep suffices
    let cloned = _.cloneDeep(remote);
    delete cloned.FETCH_HEAD;
    delete cloned.MERGE_HEAD;
    cloned.reset().checkout("master");
    return cloned;
};
