...
class Repository
{
     ...
     * @param {Object} [options]
     * @param {Repository.ResetMode} [options.mode=Repository.ResetMode.hard] - When given commit_ish, defines the way index is reset to the commit.
     */
    reset(commit_ish, options)
    {
        commit_ish = _.isString(commit_ish) ? commit_ish : "HEAD";
        options = _.extend({
            mode: Repository.ResetMode.hard
        }, options);
        ...
        if(!_.isUndefined(commit))
        {
            ...
            switch(options.mode)
            {
                case Repository.ResetMode.soft:
                    break;
                case Repository.ResetMode.hard:
                default:
