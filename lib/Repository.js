...
class Repository
{
    ...
    status()
    {
        let headCommit = this.get_commit_object(this.HEAD);
        let headIndex = (!_.isUndefined(headCommit)) ? this.get_tree_object(headCommit.tree) : {};
        let currentPaths = _.keys(this.index);
        let headPaths =  _.keys(headIndex);
        let allPaths = _.union(currentPaths, headPaths);
        let changedPaths = [];
        let that = this;
        _.forEach(allPaths, function(path){
            if(headIndex[path] !== that.index[path])
            {
