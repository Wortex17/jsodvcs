...
class Repository
{
     ...
     * @param {string} [options.out.didCommit]  - Outgoing field where it iss tored if a new commit was created
     */
    commit(message, options)
    {
        ...
        //Remove the MERGE_HEAD if there was one
    ...
    merge(commit_ish)
    {
        ...
        } else if(lca == mine)
        {
            ...
            _.forIn(uniDiff, function(contentHash, path){
                ...
                {
                    //TODO: Index Conflict, diff the contents..
                    //We should directly store the content diff3, so teh repo know where and
                    // what conflicts are still there if it cannot auto-resolve.
                    // The counter is just for this first draft of merge()
                    conflicts++;
                } else {
            ...
            if(conflicts > 0)
            {
                throw new Error("Not implemented: complex merge ");
            } else {
                //Everything was auto-resolved! Different than git, we will NOT auto-commit.
