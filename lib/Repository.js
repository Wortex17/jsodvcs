...
class Repository
{
    ...
    /**
     * Resets the current HEAD and optionally index to a previous or snapshotted state.
     * @param {Object} [options]
     * @param {Repository.ResetMode} [options.mode=Repository.ResetMode.mixed] - When given commit_ish, defines the way index and head are reset to the commit.
     * @param {String} [options.tree_ish="HEAD"] -
     */
    reset(options)
    {
        options = _.extend({
            mode: Repository.ResetMode.mixed,
            tree_ish: "HEAD"
        }, options);

        let tree = this.get_tree_object(options.tree_ish);
        if(!_.isUndefined(tree))
        {
            //Set index to reflect tree (easy, as our trees are the same as indexes)
            this.index = _.cloneDeep(tree);
        }
        return this;
    }
    /**
